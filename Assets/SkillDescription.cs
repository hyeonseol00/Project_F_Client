...
public class SkillDescription : MonoBehaviour
{
    ...
    public void useSkill(bool isMine)
    {    
        switch (Class)
        {
            case CharacterClass.SpearMan:
                activatedSkillCoroutine = StartCoroutine( ProcessSpearManSkill(SPEAR_MAN_ACTIVETIME, isMine));
                break;
            case CharacterClass.SwordMan:
                activatedSkillCoroutine = StartCoroutine(ProcessSwordManSkill(SWORD_MAN_ACTIVETIME, isMine));
                break;
            case CharacterClass.CrossbowMan:
                activatedSkillCoroutine = StartCoroutine(ProcessCrossbowManSkill(CROSSBOW_MAN_ACTIVETIME, isMine));
                break;
            case CharacterClass.HammerMan:
                activatedSkillCoroutine = StartCoroutine(ProcessHammerManSkill(HAMMER_MAN_ACTIVETIME, isMine));
                break;
            case CharacterClass.Mage:
                activatedSkillCoroutine = StartCoroutine(ProcessMageSkill(MAGE_ACTIVETIME, isMine));
                break;
            default:
                break;
        }
    ...
    // =====================클래스 스킬 시작============================
    private IEnumerator ProcessSpearManSkill(float activatingTime, bool isMine)
    {
        if (isMine)
        {
    ...
    private IEnumerator ProcessSwordManSkill(float activatingTime, bool isMine)
    {
        if (isMine)
        {
            // 스킬 활성화 코드    
    ...
    private IEnumerator ProcessCrossbowManSkill(float activatingTime, bool isMine)
    {
        if (isMine)
        {
            var originalRate = attackScript.rate;
    ...
    private IEnumerator ProcessMageSkill(float activatingTime, bool isMine)
    {
        if (isMine)
        {
            // 스킬 활성화 코드
